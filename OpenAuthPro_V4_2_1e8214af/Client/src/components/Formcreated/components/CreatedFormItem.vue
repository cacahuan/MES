<!--
 * @description: 拖拽表单表单项渲染
 * @author: liyubao | xufuxing
 * @version: 1.0
 * @updateDate:2021-07-14 审核时可以根据表单是否只读来修改、提交表单内容
-->
...
<template>
  <el-form-item :label="frmContentDataItem.name" :prop="frmContentDataItem.model">
    <template v-if="frmContentDataItem.type == 'input'">
      <el-input
        v-if="frmContentDataItem.options.dataType == 'number' || frmContentDataItem.options.dataType == 'integer' || frmContentDataItem.options.dataType == 'float'"
        :type="frmContentDataItem.options.dataType"
        v-model.number="itemVal"
        :placeholder="frmContentDataItem.options.placeholder"
        :style="{ width: frmContentDataItem.options.width }"
        :disabled="disabled"
      ></el-input>
    ...
    <template v-if="frmContentDataItem.type == 'number'">
      <el-input-number
        v-model="itemVal"
        :style="{ width: frmContentDataItem.options.width }"
        :step="frmContentDataItem.options.step"
        :min="frmContentDataItem.options.min > 0 ? frmContentDataItem.options.min : -Infinity"
        :max="frmContentDataItem.options.max > 0 ? frmContentDataItem.options.max : Infinity"
    ...
    <template v-if="frmContentDataItem.type == 'time'">
      <el-time-picker
        v-model="itemVal"
        :is-range="frmContentDataItem.options.isRange"
        :placeholder="frmContentDataItem.options.placeholder"
        :start-placeholder="frmContentDataItem.options.startPlaceholder"
        :end-placeholder="frmContentDataItem.options.endPlaceholder"
        :readonly="frmContentDataItem.options.readonly"
        :disabled="disabled"
        :editable="frmContentDataItem.options.editable"
        :clearable="frmContentDataItem.options.clearable"
        :arrowControl="frmContentDataItem.options.arrowControl"
        :value-format="frmContentDataItem.options.format"
        :style="{ width: frmContentDataItem.options.width }"
      >
      </el-time-picker>
    </template>
    ...
    <template v-if="frmContentDataItem.type == 'date'">
      <el-date-picker
        v-model="itemVal"
        :type="frmContentDataItem.options.type"
        :placeholder="frmContentDataItem.options.placeholder"
        :start-placeholder="frmContentDataItem.options.startPlaceholder"
        :end-placeholder="frmContentDataItem.options.endPlaceholder"
        :readonly="frmContentDataItem.options.readonly"
        ...
        :editable="frmContentDataItem.options.editable"
        :clearable="frmContentDataItem.options.clearable"
        :value-format="frmContentDataItem.options.timestamp ? 'timestamp' : frmContentDataItem.options.format"
        :format="frmContentDataItem.options.format"
        :style="{ width: frmContentDataItem.options.width }"
      >
      </el-date-picker>
    </template>
    ...
    <template v-if="frmContentDataItem.type === 'imgupload'">
      <Upload
        ...
        :disabled="disabled"
        :style="{ width: frmContentDataItem.options.width }"
        :width="frmContentDataItem.options.size.width"
        :height="frmContentDataItem.options.size.height"
        :token="frmContentDataItem.options.token"
        :domain="frmContentDataItem.options.domain"
        :multiple="frmContentDataItem.options.multiple"
        :length="frmContentDataItem.options.length"
        :is-qiniu="frmContentDataItem.options.isQiniu"
        :is-delete="frmContentDataItem.options.isDelete"
        :min="frmContentDataItem.options.min"
        :is-edit="frmContentDataItem.options.isEdit"
        :action="frmContentDataItem.options.action"
      >
      </Upload>
    </template>
...
export default {
  ...
  created() {
    if (this.frmContentDataItem.options.remote && this.remote[this.frmContentDataItem.options.remoteFunc]) {
    ...
    if (this.frmContentDataItem.type === 'imgupload' && this.frmContentDataItem.options.isQiniu) {
