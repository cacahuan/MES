<template>
  <div class="img-uplaod-container" :id="uploadId">
    <draggable class="drag-img-list" v-model="fileList" v-bind="{group: uploadId, ghostClass: 'ghost', animation: 200}"
      :no-transition-on-drag="true">
      <div :id="item.key" :style="{width: width+'px', height: height+'px'}"
        ...
        <div class="uplaod-action" :style="{height: miniWidth / 4 + 'px'}">
          <i :class="item.isImg ? 'el-icon-view' : 'el-icon-download'" :title="item.isImg ? '预览' : '下载'"
            @click="handlePreviewFile(item)" :style="{'font-size': miniWidth/8+'px'}"></i>
          <i v-if="isEdit && !disabled" class="el-icon-refresh" :title="'替换'" @click="handleEdit(item.key)"
            :style="{'font-size': miniWidth/8+'px'}"></i>
          <i v-if="isDelete && fileList.length > min && !disabled" class="el-icon-delete" :title="'删除'"
            @click="handleRemove(item.key)" :style="{'font-size': miniWidth/8+'px'}"></i>
        </div>
      </div>
    </draggable>

    <div class="el-upload el-upload--picture-card" :class="{'is-disabled': disabled}"
      v-show="(!isQiniu || (isQiniu && token)) && fileList.length < length"
      :style="{width: width+'px', height: height+'px'}" @click.self="handleAdd">
      <i class="el-icon-plus" @click.self="handleAdd"
        :style="{fontSize:miniWidth/4+'px',marginTop: (-miniWidth/8)+'px', marginLeft: (-miniWidth/8)+'px'}"></i>
     <input v-if="multiple" multiple ref="uploadInput" @change="handleChange" type="file"
        :style="{width: 0, height: 0}" name="files" class="el-upload__input upload-input">
      <input v-else ref="uploadInput" @change="handleChange" type="file" :style="{width:0, height: 0}" name="files"
      ...
      uplaodAction(res, file, key) {
          ...
          if (xhr.readyState === 4) {
            ...
            if (resData && resData.result && resData.result.length > 0) {
              this.$set(this.fileList, this.fileList.findIndex(item => item.key === key), {
                ...
                url: `${process.env.VUE_APP_BASE_IMG_URL}/${resData.result[0].filePath}`,
                isImg: ['.jpg', '.jpeg', '.png', '.svg', '.gif'].indexOf(resData.result[0].fileType) >= 0 ||
                  false,
                percent: 100
              })
              setTimeout(() => {
        ...
        xhr.onprogress = (res) => {
          ...
          if (res.total && res.loaded) {
            ...
            this.$set(this.fileList[this.fileList.findIndex(item => item.key === key)], 'percent', res.loaded / res
              .total * 100)
          }
        }
      },
      uplaodAction2() {  //七牛云上传处理，暂时不用
        // const _this = this
